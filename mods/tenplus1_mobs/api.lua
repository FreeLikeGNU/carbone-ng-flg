...
tenplus1_mobs = {}
tenplus1_mobs.mod = "redo"

-- Initial settings check
local damage_enabled = minetest.setting_getbool("enable_damage")
local peaceful_only = minetest.setting_getbool("only_peaceful_mobs")
local enable_blood = minetest.setting_getbool("mobs_enable_blood") or true
tenplus1_mobs.protected = tonumber(minetest.setting_get("mobs_spawn_protected")) or 0

function tenplus1_mobs:register_mob(name, def)
			...
			elseif self.state == "attack" and self.attack_type == "explode" then 
				...
				if self.attack.dist > 3 then
					...
					if self.timer > 3 then
						...
						self.object:remove()
						pos.y = pos.y - 1
						tenplus1_mobs:explosion(pos, 2, 0, 1, self.sounds.explode)
					end
				end
...
tenplus1_mobs.spawning_mobs = {}

function tenplus1_mobs:spawn_specific(name, nodes, neighbors, min_light, max_light, interval, chance, active_object_count, min_height, max_height)
	tenplus1_mobs.spawning_mobs[name] = true
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not tenplus1_mobs.spawning_mobs[name] then
			--or not pos then
				return
			end
...
function tenplus1_mobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height)
	tenplus1_mobs:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30, chance, active_object_count, -31000, max_height)
end
...
-- explosion
function tenplus1_mobs:explosion(pos, radius, fire, smoke, sound)
...
function tenplus1_mobs:register_arrow(name, def)
...
-- Spawn Egg
function tenplus1_mobs:register_egg(mob, desc, background, addegg)
...
function tenplus1_mobs:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and not self.child then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			if tool:is_empty() then
				chance = chance_hand
			elseif tool:get_name() == "tenplus1_mobs:net" then
				chance = chance_net
				tool:add_wear(4000) -- 17 uses
				clicker:set_wielded_item(tool)
			elseif tool:get_name() == "tenplus1_mobs:magic_lasso" then
				-- pick up if owner
				chance = chance_lasso
